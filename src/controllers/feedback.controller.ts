...
export class FeedbackController {
  ...
  async findBy(
    ...
    const sql = await this.feedbackRepository.execute(
      `select * from ${process.env.DB_SCHEMA}.tgt_lead_feedback where property_id = '${property_id}' order by updated_on DESC
      `
    )
    return sql;
